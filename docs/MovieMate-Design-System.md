...
| `statTileNumber` | 28sp | عدد آماری وقتی چیز دیگری همان ردیف/کارت را با آن شریک است — کارت سه‌تایی (Matches/Watched/Streak) یا درصد سازگاری کنار یک جمله در `CompatibilityRow` — نه عدد قهرمانِ تنها (v10.2.1) |
...
بدون تغییر در `tokens.json` — یک ممیزی سیستماتیک روی همه‌ی استفاده‌های
`MovieMateType.*` در اپ (همان الگوی باگ‌های `statCaption`/`statNumber` قبلی)
دو مورد دیگر پیدا کرد.

**اصلاحات**
- حرف اول آواتار بدون عکس همیشه با `statCaption` (۲۴sp) رندر می‌شد، صرف‌نظر
  از این‌که آواتار ۴۴dp (پیش‌فرض)، ۶۴dp (صفحه‌ی انتظار)، یا ۹۶dp (ویرایش
  پروفایل) باشد — در ۴۴dp خیلی بزرگ، در ۹۶dp خیلی کوچک. `Avatar` حالا سایز
  حرف را از روی پارامتر `size` خودش محاسبه می‌کند (۰.۴ برابر قطر دایره)، نه
  یک نقش ثابت — چون این کامپوننت، برخلاف یک صفحه، در چند اندازه‌ی متفاوت
  استفاده می‌شود.
- `CompatibilityRow` در Us از `statNumber` (۴۴sp، برای عدد تنها) برای درصد
  استفاده می‌کرد، در حالی که آن عدد کنار یک جمله‌ی کامل در همان ردیف جا
  می‌گرفت — همان الگوی ازدحامی که `statTileNumber` را برای کارت‌های سه‌تایی
  ساخت. حالا از `statTileNumber` (۲۸sp) استفاده می‌کند؛ توضیح این نقش در
  بخش ۵ به‌روزرسانی شد تا هر دو زمینه (کارت سه‌تایی، ردیف کنار یک جمله) را
  پوشش دهد.
