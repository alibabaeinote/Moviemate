...
### ✅ سه تصمیم محصولی — تصمیم‌گیری شد (۲۰۲۶-۰۹-۱۲)

| مورد | تصمیم |
|---|---|
| **تعریف streak** | سند می‌گفت «روزهای متوالی»، ولی PRD §۲ می‌گوید کاربر هدف هفته‌ای ۱-۲ بار فیلم می‌بیند. با تعریف تحت‌اللفظی، streak برای هر کاربر واقعی همیشه ۱ می‌ماند. **تایید شد: همان پنجره‌ی مهلت ۷ روزه** (`PRODUCT_CONFIG.streakGraceDays = 7`) می‌ماند — این دیگر یک انحراف نیست، رفتار مورد نظر است. |
| **نوتیف «شریکت فیلم را دید، امتیاز بده»** | **تایید شد: ساخته شد.** نوع هشتم نوتیف (`partner_watched`) اضافه شد — وقتی یکی «We watched it» را می‌زند، شریکش پیام می‌گیرد که برود امتیاز بدهد. زیر همان سقف روزانه‌ی نوتیف‌های غیرضروری (frequency cap) قرار دارد، نه معاف از آن. نیازمند فیلد جدید `match.watchedConfirmedBy` بود تا بدانیم کدام طرف را نباید نوتیف کنیم. |
| **آمار journey و compatibility** | **تایید شد: تعریف و ساخته شد.** journey = روند هفتگی تعداد فیلم‌های تماشاشده (۶ هفته‌ی اخیر). compatibility = میانگین شباهت امتیازهای Taste Dial (۱۰۰ − اختلاف) روی فیلم‌هایی که هر دو نفر امتیاز داده‌اند؛ زیر ۳ فیلم مشترک عدد نشان داده نمی‌شود (به‌جای عدد بی‌معنی). منطق در `UsStatsMath.kt` (تست‌شده)، محاسبه سمت کلاینت مثل `pairTotals`. |

### ✅ شکاف بسته‌شده (۲۰۲۶-۰۹-۱۲)

| شکاف | راه‌حل |
|---|---|
| `generateDailyMatch` تست یکپارچه نداشت | `generateMatchForPair` یک پارامتر اختیاری `buildPool` گرفت (پیش‌فرض همون `buildCandidatePool` واقعی)؛ تست‌ها یک استخر ثابت تزریق می‌کنند تا هرگز به TMDB نخورند. سه تست جدید در `functions/integration/generateDailyMatch.test.ts` (جمع یکپارچه: ۶۲). |
...
| Us (streak / journey / compatibility) | ✅ هر سه ساخته شدند |
